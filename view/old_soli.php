<?php
	...
	if($_POST['acao'] === 'getUltimoEstado'){
		//setor ж na saida de materiais e a coferencia ж da conferencia pos procedimento
	...
	// verifica se jр existe solicitaусo do setor; se nсo existir, salva uma nova e retorna o ID
	if (isset($_GET['nova'])) {
		$res = SolicitacoesController::getSolicitacoes("ses_idsetor = " . $_GET['nova']);
		if($res) { // se jр existir, apenas retorna resposta
	...
	// busca produto para inserir na solicitaусo
	if ($_POST['acao'] == 'buscar' && isset($_POST["setor"]) && (isset($_POST["qrcode"])) && (!empty($_POST['qrcode']) || $_POST['qrcode'] != '' || $_POST['qrcode'] != ' ')){
		if($_POST['chProduto'] == 'pn'){
			...
			if ($produto->pro_id != ""){
				...
				// se produto jр estiver incluso na solicitaусo
				...
				// se produto jр estiver incluso na solicitaусo
				// !empty($_POST['txQtde'])
				//verificar se o produto tem qtde se tiver ele deixa passar, se nсo ele dр como repetido
				...
				$descarte = OcorrenciasProdutosController::getDescarteByProduto($produto->pro_id);
				// verifica se produto contжm ocorrЖncia que anula reuso
	...
	if (isset($_POST['acao']) && $_POST['acao'] != 'buscar'){
		if ($_POST['acao'] == 'slProd'){
			...
			if($_POST['qtde'] > 1){
				...
				//numero sequencial, quando o produto for novo ele vai comeуar de 1 se jр estiver com alguns no banco, ele contarр apartir do ultimo inserido
				...
				for ($i; $i < $qtdep; $i++) {
					...
					$_POST['idProduto'] = $gp->pro_id;
					$_POST['qrcode'] = $gp->pro_qrcode;
					$_POST['qtdeproduct'] = $gp->pro_qtde;
...
?>
	...
	<table class="table table-hover">
		...
		<tr class="cs-tr">
			<td colspan="4"><h4 id="comum">Produtos Comuns <i class="fas fa-tag cs-fa"></i></h4></td>
			<td>
		...
		<tr class="cs-tr">
			<td colspan="4"><h4 id="consignados">Produtos Consignados <i class="fas fa-user-tag cs-fa"></h4></td>
			<td><span id="totalConsignado" style="float:right !important; margin:5px !important;"></span></td>
		</tr>
		<tbody id="lista_consig">
			<!-- ConteЩdo da lista auto fill solicitaушes.js-->
		</tbody>
	</table>
		...
		<div id="tlaProduto" class="modal hide fade">
			...
			<div class="btn-group" data-toggle="buttons-radio" style="margin-top:15px; display: flex; width:270px;">
					...
					<label for="pn" style="width:70%;">
						...
						<button type="button" class="btn btn-primary active hover-button" id="bt-pn">
					...
					<label for="pc" style="width:70%;">
						...
						<button type="button" class="btn btn-primary hover-button"  id="bt-pc">
			...
			<div class="modal-body" style="width: 2050px; height: 4050px;">
				...
				<div id="boxQtde">
					...
					Qtde produto:
					<input type="text" name="qtdeproduct" id="qtdeProduct" style="text-transform:;" readonly class="input-mini"/>
					</span>
					<br><br>
				...
				<div id="divRestoItem" class="hide">
					...
					<br>
					<!--label>Setor: <span id="txSetor"></span></label>
					<br-->
					<!--<label>
						Mжtodo de esterilizaусo: 
						<?php 
							//echo SolicitacoesHelper::populaComboMEsterilizacao();
						?>
					</label>-->
					<label>
						Responsрvel tжcnico pelo preparo: 
						<?php 
					...
					<div id = 'hidde_teste'>
					<input type="radio"   name="estandar" value="0" id = "ventilatorio" class="tipodetergente" <?php echo $_SESSION['estandar'] == 0 ? "checked='checked'" : ''?>> Ventilatзrio<br><br>
			...
			<div class="modal-footer">
				...
				<a id="lbProdutoinvertido" class="hide" style="color: red; font-weight: bold;">O produto nсo ж <span class="type-product"></span> ou </a>
				<a id="lbProdutoNaoCadastrado" class="hide" style="color: red; font-weight: bold;">o produto nсo foi cadastrado! </a>
				...
				<a href="#" class="btn btn-danger hide" id="btOcorrenciaPro"><i class="icon-edit icon-white"></i> Lanуar ocorrЖncia</a>
...
function printDiv(close, divName) {
	...
	<div id="telaDetail" class="modal hide fade">
		<div id="divimprimi">
			<div class="modal-header">
	...
	<form id="formListaProdutos">
		<div id="telaListaProdutos" class="modal hide fade" data-keyboard="false" data-backdrop="static">
			...
			<div class="modal-body" style="width: 530px; height: 4050px;">
				  ...
				  <div class="boxinfo">
					...
					<div class="layer">
						<div class="tooltip_info">
							...
							<p>ConferЖnte: <span id="conferente"></span></p>
						</div>
					</div>
				  </div>				
				<br>
				...
				<h5>Incluir produtos de sua composiусo na solicitaусo</h5>
			...
			<div class="modal-footer">
				...
				<div>
					...
					<a href="#" class="btn btn-danger hide" id="btOcorrenciaFilho"><i class="icon-edit icon-white"></i> Lanуar ocorrЖncia</a>
	...
	<form id="formPermissao">
		<div id="telaPermissao" class="modal hide fade" data-keyboard="false" data-backdrop="static">
			...
			<div class="modal-body" style="width: 530px; height: 4050px;">
				<b>Solicite a autorizaусo do responsрvel tжcnico do turno atual</b>
				<br><br>
				<label>
					Login:
					<input type="text" id="responsavel" class="input-large" value="LARISSABDN" onkeypress="return noenter()" autofocus>
				</label>
				<label>
					Senha:
					<input type="password" id="password" class="input-large" value="B5FA8" onkeypress="return noenter()" autofocus>
				</label>
			</div>
			<div class="modal-footer">
	...
	<form id="formLancarOcorrencia">
		<div id="tlaLancarOcorrencia" class="modal hide fade">
			...
			<div class="modal-body" style="width: 530px; height: 500px;">
				...
				<label>Descriусo: <span id="descricaoOcorrencia"></span></label>
				<br>
