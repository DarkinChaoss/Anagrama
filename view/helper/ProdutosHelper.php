<?php
	class ProdutosHelper{
	    ...
	    public static function geraCodigoProduto(){
	        $codigo = date("ymdHis") . $_SESSION['usu_id'];
	        return $codigo;
	    }
		
		public static function listaProdutos($buscar, $pag = 0, $descart){
			$html = "";
			if($pag != 0){
				$inicio = ($pag - 1) * 10;
				$qtde = 10;
			...
			if($descart != "S")
				...
				$where = "";
			foreach(ProdutosController::getProdutosBuscar($buscar, $limit, $where) as $p ){
				...
				$p->pro_qtde != 0 && $p->pro_qtde != null ? $reuso = '-' : $reuso =  $reuso . "/" . $p->pro_maxqtdprocessamento;
				$html .= "<tr>
		...
		public static function paginacao($buscar, $pag, $descart){
			if($descart != "S")
				...
				$where = "";
			$n = ProdutosController::getProdutosBuscarCount($buscar, "", $where);
			$pags = ceil($n / 10);
		...
		public static function listaProdutosEtiquetagem(){
			$html = "<script src='js/produtos.js'></script>";
		...
		// Permissão para alteração de produtos - usu_id	     
                 $arr_usuPer = array(52, 51, 53, 54, 55, 56, 58, 59, 61, 62, 65, 66, 67, 69, 112, 119, 126, 89, 134);

				 $conteudo = "<script type='text/javascript'>
						console.log('".$teste."');
						$('#txId').val('" . $obj->pro_id . "');
						$('#txQrcode').val('" . $obj->pro_qrcode . "');
						" . (($obj->pro_qtde != null && $obj->pro_qtde != 0) ? "$('#ckQtde').attr('checked', 'true');" : "") . "
						...
						" . (($obj->pro_composto == 1) ? "$('#ckCompostockComposto').attr('checked', 'true');" : "") . "
