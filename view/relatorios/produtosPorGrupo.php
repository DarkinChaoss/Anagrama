...
?>
	...
	<h1>Relatório de Produtos por grupo de material</h1>
	...
	<div id="divPrint">
		...
		if($_REQUEST['gerar'] == 1){
			...
			$html .= "	<div class='onlyPrint'>
							...
							<h4>Relatório de Produtos por grupo de material</h4>
						</div>
						<br>
			...
			if($_REQUEST['tipo'] == "a"){ // ANALÍTICO
				...
				$totalGrupo = 0;
				$totalGeral = 0;
				foreach (ProdutosController::relProdutos("gma_nome LIKE '%" . $_REQUEST['filtro'] . "%'", "gma_nome, pro_nome", true) as $pro){					
					...
					// total em cada grupo
					if($grupoAnterior != $pro->pro_gma_nome && $i != 0){
						$html .= "	<tr>
										<td colspan='6' class='bold right'>Total de produtos em " . $grupoAnterior . ": </td>
										<td class='bold left'>" . $totalGrupo . "</td>
									</tr>";
						$totalGrupo = 0;
					}
					$html .= "	<tr>
					...
					$grupoAnterior = $pro->pro_gma_nome;
					++$totalGrupo;
					++$totalGeral;
					++$i;
				}
				...
				$totalGrupo = 0;
				$totalGeral = 0;
				foreach (ProdutosController::relProdutos("gma_nome LIKE '%" . $_REQUEST['filtro'] . "%'", "gma_nome, pro_nome") as $pro){
