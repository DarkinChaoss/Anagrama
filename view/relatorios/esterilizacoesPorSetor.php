...
?>
	...
	<h1>Relatório de Esterilizações por setor</h1>
			...
			<div class="span4">
				...
				<?php
    				foreach (SetoresController::getSetores() as $objSetor){
    				    if( $objSetor->set_fazsolicitacao == 'S' ){
	...
	<div id="divPrint">
		...
		if($_REQUEST['gerar'] == 1){
			...
			$html .= "	<div class='onlyPrint'>
							...
							<h4>Relatório de Esterilizações por setor</h4>
						</div>
						<br>
			...
			if($_REQUEST['tipo'] == "a"){ // ANALÍTICO
				...
				foreach (SolicitacoesController::relSolicitacao("set_id = ".$_REQUEST['filtro']." AND (" . $data1 . $and . $data2 . $vazio . ") AND iso_dataesterilizacao <> '0000-00-00'", "iso_dataesterilizacao, iso_horaesterilizacao, set_nome") as $ses){
				...
				$and = (($_REQUEST['data1'] != "" && $_REQUEST['data2'] != "") ? " AND " : "");
				$vazio = (($_REQUEST['data1'] == "" && $_REQUEST['data2'] == "") ? "ses_del IS NULL" : "");
				foreach (SolicitacoesController::relSolicitacao("set_id = ".$_REQUEST['filtro']." AND (" . $data1 . $and . $data2 . $vazio . ") AND iso_dataesterilizacao <> '0000-00-00'", "iso_dataesterilizacao, set_nome") as $ses){
					/////$set = SetoresController::getSetor($ses->ses_idsetor);
					//$itens = ItensSolicitacoesController::getItens("iso_idses = " . $ses->ses_id);
					//$dia = split(" ", $ses->ses_datasaida);
					//$dia = DefaultHelper::converte_data($dia[0]);
