...
?>
	...
	<h1>Relatório de Produtos por setor</h1>
	...
	<div id="divPrint">
		...
		if($_REQUEST['gerar'] == 1){
			...
			$html .= "	<div class='onlyPrint'>
							...
							<h4>Relatório de Produtos por setor</h4>
						</div>
						<br>
			...
			if($_REQUEST['tipo'] == "a"){ // ANALÍTICO
				...
				$totalSetor = 0;
				$totalGeral = 0;
				foreach (ProdutosController::relProdutos("set_nome LIKE '%" . $_REQUEST['filtro'] . "%'", "set_nome, pro_nome") as $pro){
					...
					// total em cada setor
					if($setorAnterior != $set->set_nome && $i != 0){
						$html .= "	<tr>
										<td colspan='6' class='bold right'>Total de produtos em " . $setorAnterior . ": </td>
