	...
	<form>
		...
		<div class="btn-group" data-toggle="buttons-radio" style="margin-top:px; display: flex; width:270px;">
				...
				<label for="pn" style="width:70%; margin-right: 10px">
					<span style="background: rgba(0,0,0,.0); width:78% !important; height:30px; display:none; position: absolute"></span>
					<a class="btn btn-primary hidden hover-button" id="bt-pn" style="display:none !important;">
				...
				<label for="pc" style="width:70%;">
					<span style="background: rgba(0,0,0,.0); width:60% !important; height:90%; display:none; position: absolute"></span>
					<a class="btn btn-primary hover-button hidden"  id="bt-pc" style="display:none !important;">
	...
	<div id="divPrint">
		...
		if($_REQUEST['gerar'] == 1){
			...
			if($_REQUEST['chProduto'] == 'pn'){
					// busca produto
				if(count($qrcodenew) == 3){
					$qrcodenew = explode(".", $_REQUEST['filtro']);
					$qrcodeN = $qrcodenew[0].'.'.$qrcodenew[1];
					$res = ProdutosController::getProdutos("pro_qrcode = '" . $qrcodeN . "'");
				}
				else{
				...
				if(!empty($pro->pro_id)){
					...
					$userN = SaidaMateriaisController::getUserNamebyiduser($pro->pro_idusuario);
					...
					if(count($qrcodenew) == 3){
						$lote = $qrcodenew[2];
						$arrIsa = ItensSaidaController::getItensSaida("isa_idproduto = " . $pro->pro_id.' and isa_loteref= '.$lote.' ');
					}
					else{
					...
					for($i = 0; $i < sizeof($arrIsa); $i++){ // enquanto hover saídas...


						error_log('<'.$arrIsa[$i]->isa_idsaida.'>');

						if(count($arrIsa[$i]->pro_qrcode) >= 2){
							$sma = SaidaMateriaisController::getSaidaMateriais($arrIsa[$i]->isa_idsaida);
						}
						else{
							$sma = SaidaMateriaisController::getSaidaMateriais($arrIsa[$i]->isa_idsaida, $arrIsa[$i]->loteref);
						}

						$userName = SaidaMateriaisController::getFullUserName($arrIsa[$i]->isa_idsaida);

						$retiradoPor = $sma->sma_retirado_por == 'sistema' ? '' : 'Retirado por: <b>'. strtoupper($sma->sma_retirado_por).'</b>';


						$setor_destino = ItensSaidaController::getItemBySaidaEProduto($arrIsa[$i]->isa_idsaida, $pro->pro_id);


						//para produtos com qte não está trazendo as outras linhas;
						/*if($setor_anterior == $setor_destino['isa_idsetordestino']){
							if($sma->sma_tiposaida == 'S'){
						...
						$setDest = SetoresController::getSetor($setor_destino['isa_idsetordestino']);
						//print_r($setor_destino['isa_idsetororigem']);
						$setOrigin = SetoresController::getSetor($setor_destino['isa_idsetorigem']);
						//print_r($setor_destino);
						//die($setDest);
						$arr[$iArr]['data'] = $arrIsa[$i]->isa_data;
						$arr[$iArr]['empresa'] = "-";

						$textoRetirada = ($arrIsa[$i]->isa_reuso == "") ? "PRIMEIRO USO:" : "<p style='margin:0 !important; padding:0 !important;'>RETIRADA:</p>";
						$inf = "Qrcode : <b> {$sma->qrcodenew}.{$sma->loteref}</b>  &nbsp;&nbsp; &nbsp; Qtde:<b> {$sma->qte}</b>";
						...
						if( $sma->sma_tiposaida === 'T' ){
							...
							if($setDest->set_nome == 'CME'){
								$arr[$iArr]['texto'] = "Entrada na <b>CME</b>";
								$retiradoPor = '';
							}else{
								$arr[$iArr]['texto'] = "Transferencia de <b> {$setOrigin->set_nome}</b>  para  <b>{$setDest->set_nome}</b> ";
							}
							
							$arr[$iArr]['retiradoPor'] = $retiradoPor;
							$arr[$iArr]['inf'] = $inf;
							if($sma->sma_retirado_por == 'sistema'){
								$arr[$iArr]['transferidoPor'] = 'Conferido por: <b>'.$userName.'</b>';
							}else{
								$arr[$iArr]['transferidoPor'] = 'Entregue por: <b>'.$userName.'</b>';
							}
							
						}
						else{
							$arr[$iArr]['texto'] = 	"{$textoRetirada} PRONTUÁRIO: <b>{$sma->sma_prontuario}</b>, " .
													"PACIENTE: <b>{$sma->sma_paciente}</b>, SALA: <b>{$arrIsa[$i]->isa_sala}</b>, SETOR: <b>".$setDest->set_nome ."</b>". 
													(($arrIsa[$i]->isa_obs != "") ? "<br><i>Obs.: <b>" . 
													$arrIsa[$i]->isa_obs . "</b></i>" : "");
							$arr[$iArr]['inf'] = "Qrcode : <b>  {$sma->qrcodenew}.{$sma->loteref} </b>  &nbsp;&nbsp; &nbsp; Qtde:<b>{$sma->qte}</b> ";
						}
						$iArr++;
					...
					$order = "iso_dataesterilizacao, iso_horaesterilizacao";
					if(count($qrcodenew) == 3){
						$lote = $qrcodenew[2];
						$where = "iso_idproduto = " . $pro->pro_id." and iso.iso_lote_referencia = {$lote}";
					}
					else{
						$where = "iso_idproduto = " . $pro->pro_id;
					}
					
					
					foreach (ItensSolicitacoesController::relItens($where,$order) as $iso){
						...
						while($j < sizeof($arr) && $iso->iso_dataesterilizacao." ".$iso->iso_horaesterilizacao > $arr[$j]['data']){
							if($arr[$j]['empresa'] == "-"){
								...
								if(count($qrcodenew) >= 2){
									$html .= "	<tr>
													<td>" . DefaultHelper::converte_data($arr[$j]['data']) . "</td>
													<td colspan='4'>" . $arr[$j]['texto'] . "</td>
													<td colspan='2'>" .  $arr[$j]['inf']. "</td>
													<td colspan='2'>" . $arr[$j]['transferidoPor'] . '&nbsp; &nbsp; &nbsp; &nbsp;'. $arr[$j]['retiradoPor'] .  "</td>
												</tr>";
								}else{															
									$html .= "	<tr>
													<td>" . DefaultHelper::converte_data($arr[$j]['data']) . "</td>
													<td colspan='3'>" . $arr[$j]['texto']. "</td>
													<td colspan='1'>" . $arr[$j]['transferidoPor'] . "</td>
													<td colspan='2'>" . $arr[$j]['retiradoPor'] . "</td>
												</tr>";
								}
				...
				if(!empty($vcons->pro_id)){
				  ...
				  $html .= "
					...
					for($i = 0; $i < sizeof($arrIsa); $i++){
						...
						if( $sma->sma_tiposaida === 'T' ){
							$arr[$iArr]['texto'] = "Transferincia para: {$set->set_nome} ";
						}
						else{
