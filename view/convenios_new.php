<?php
	if (isset($_POST['nome'])) {
		if(empty($_POST['id']))
			$res = ConveniosController::insert($_POST);
		else
			$res = ConveniosController::update($_POST);
		if($res)
			die("OK");
		else
			die("ERRO");
	...
	if(isset($_GET['delete'])){
		if(ConveniosController::delete($_GET['id']))
			die("OK");
		else
			die("ERRO");
	}
...
?>
	...
	<form id="formConvenio">
		...
		<br>
		<select name="ativo" id="slAtivo" class="input-small">
...
<?php
	if(isset($_GET['populate'])){
		$convenio = ConveniosController::getConvenio($_GET['id']);
		echo ConveniosHelper::populaCampos($convenio);
	}
