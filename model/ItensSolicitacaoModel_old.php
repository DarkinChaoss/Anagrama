<?php
	class ItensSolicitacaoModel extends Conexao{
		...
		public $pro_qrcode;
		public $pro_id;
		// referente а atualizaз~~ao de conferencia de material -> saida da esterilizacao
		...
		public function insert(){
			...
			if(count($ja) > 0){
				error_log("BLOQUEOU ADD ITEM! JБ TEM.");
				return false;
			} else {
				...
				if($res) {
					// log
					$this->log_acao = "Inserзгo: item " . $id . " pertencente a solicitaзгo " . $this->iso_idses . " em tmsd_itenssolicitacao.";
					$this->gravaLog();
					$res = $id;
					//
				}
		...
		public function update(){
			...
			if($res) {
				// log
				$this->log_acao = "Atualizaзгo: item " . $this->iso_id . " pertencente a solicitaзгo " . $this->iso_idses . " em tmsd_usuarios.";
				$this->gravaLog();
				//
			}
			return $res;
		...
		public function updateConferencia(){
			...
			if($res) {
				// log
				$this->log_acao = "Atualizaзгo: item " . $this->iso_id . " conferкncia da solicitaзгo de esterilizaзгo " . $this->iso_idses . " em tmsd_itenssolicitacao.";
				$this->gravaLog();
				//
			}
			return $res;
		...
		public function delete($id){
			...
			if($res) {
				// log
				$this->log_acao = "Exclusгo: registro " . $id . " em tmsd_itenssolicitacao.";
				$this->gravaLog();
				//
			}
			return $res;
		...
		public function selectItem($iso_id){
			//echo $iso_id;
			$sql = "SELECT * FROM tmsd_itenssolicitacao AS iso
		...
		// mesma busca que selectItem, porйm no banco Sterilab, para cruzamento de dados
		...
		//aqui pega os produtos da compoisзгo da caixa e os q1ue estгo na solicitaзгo jб 
		...
		//seleciona os que NГO estгo na solicitaзгo		
		...
		//seleciona os que NГO estгo na solicitaзгo	quando nao foi inserindo nenhum na caixa
		...
		//seleciona os que estгo na solicitaзгo
		...
		//Produtos da tela de solicitaзгo
		public function selectItensTelaSolicitacao($ses_id){
			...
			while($row = mysql_fetch_array($res, MYSQL_ASSOC)){
				...
				// Irб esconder os produtos filhos de compostos ao trazer do banco.
				if($this->filho_de_composto($row['iso_idproduto'])){
					//continue; // Se descomentar nгo mostra os filhos na solicitaзгo
					$obj->color = '#ececec';
		...
		//conta intens consignados da solicitaзгo
		//cleverson
		//conta itens da solicitaзгo
		...
		public function selectAllOrder($where, $order){
			if(isset($where))
				$where = "AND " . $where;
					$sql = "SELECT CONCAT(pro.pro_qrcode,'.',iso.iso_lote_referencia) AS qrcodenew, iso.*,pro.* FROM tmsd_itenssolicitacao AS iso
					INNER JOIN tmss_produto AS pro ON (iso_idproduto = pro_id)
					WHERE iso_del IS NULL AND iso_consignado = 0 " . $where . "
					ORDER BY " . $order;
					error_log('slq>>>'.$sql);
		...
		//aqui provavelmente faz a contagem dos produtos que foram inseridos e conferidos na solicitaзгo
		...
		// mesma busca que selectAllOrder, porйm no banco Sterilab, para cruzamento de dados
		...
		public function setDataLimite($id, $data){
		    ...
		    if($res) {
		        // log
		        $this->log_acao = "Atualizaзгo: (iso_datalimite) item " . $id . " em tmsd_itenssolicitacao.";
		        $this->gravaLog();
		        //
		    }
		    return $res;
